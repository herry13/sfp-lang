...
let make_ts typeEnvInit flatStoreInit typeEnvGoal flatStoreGoal typeValues =
	let typeObject = Syntax.TBasic Syntax.TObject in
	let type_of_variable ref =
		match (Type.type_of ref typeEnvInit), (Type.type_of ref typeEnvGoal)
			with
	...
	let result = MapRef.fold (
			fun r v acc ->
				...
				| Syntax.TBasic Syntax.TSchema (_, _) ->
					...
					let variable = make r acc.nextVariableIndex [|value|]
						value value
					in
					{
